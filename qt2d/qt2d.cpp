...
void qt2d::Paint_zx()
{
    ...
    int pointx=35,pointy=280;//确定坐标轴起点坐标，这里定义(35,280)s
    int width=580-pointx,height=260;//确定坐标轴宽度跟高度 上文定义画布为600X300，宽高依此而定。
        srand(time(NULL));
        //获得数据中最大值和最小值、平均数
        int n=30;//n为数据个数
...
void qt2d::paintEvent(QPaintEvent *event)
{
#if 1
    ...
    painter.setPen(QPen(Qt::blue,9,Qt::SolidLine,Qt::RoundCap));
    painter.drawLine(QPoint(20,30),QPoint(200,30));
#endif
}
